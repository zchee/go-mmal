...
func main() {
	...
	if err := mmal.WrapperCreate(&cpCamera, mmal.ComponentDefaultCamera); err != nil {
		log.Fatalf("go-picamera: %v", err)
	}
	var cpVideoRenderor mmal.WrapperType
	if err := mmal.WrapperCreate(&cpVideoRenderor, mmal.ComponentDefaultVideoRendereu); err != nil {
		log.Fatalf("go-picamera: %v", err)
	}
	...
	if err := mmal.PortParameterSet(&input, dr.Hdr()); err != nil {
		log.Fatalf("go-picamera: %v", err)
	}
	...
	if err := mmal.ConnectionCreate(&connection, &cameraOutput, &videoInput, mmal.ConnectionFlagTunnelling|mmal.ConnectionFlagAllocationOnInput); err != nil {
		log.Fatalf("go-picamera: %v", err)
	}
	// and enable it
	if err := mmal.ConnectionEnable(&connection); err != nil {
		log.Fatalf("go-picamera: %v", err)
	}
